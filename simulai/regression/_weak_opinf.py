...
# Operator inference using quadratic non-linearity using weak formulation for derivatives
class WeakOpInf:
    def __init__(
        self,
        forcing: str = None,
    ...
    def _serial_operators_construction_dispatcher(
        ...
        for ii, (i_chunk, t_chunk, time_chunk, f_chunk) in enumerate(
            zip(input_chunks, target_chunks, time_data,  forcing_chunks)
        ):
            sys.stdout.write(
            ...
            D_o_ii, R_matrix_ii = self._construct_operators(
                input_data=i_chunk, target_data=t_chunk, time_data=time_chunk, forcing_data=f_chunk
            )
    ...
    def _generate_data_matrices(
        ...
        **kwargs,
        ...
        V, Vp, grid = self.test_function.build_V_Vp(time_data)
    ...
    def _construct_operators(
        ...
        **kwargs,
        ...
        # Generating the data-driven matrices
        D, Res_matrix = self._generate_data_matrices(
            input_data=input_data, target_data=target_data, time_data=time_data, forcing_data=forcing_data
        )
